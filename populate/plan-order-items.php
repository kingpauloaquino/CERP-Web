  ...
  $startpoint = $limit * ($page - 1);
	...
	if($_GET['t'] == 'P/O') {
		$query = $DB->Fetch('purchase_order_items', array(
	               'columns' => 'purchase_order_items.id, purchase_order_items.purchase_order_id AS order_id, products.id AS item_id, products.product_code AS code, 
	               								products.description, item_costs.cost AS item_price, lookups.code AS unit, purchase_order_items.item_type AS item_type, 
		...
		$query = $DB->Fetch('work_order_items', array(
	               'columns' => 'work_order_items.id, work_order_items.work_order_id AS order_id, products.id AS item_id, products.product_code AS code, 
																products.description, item_costs.cost AS item_price, lookups.code AS unit, work_order_items.quantity AS quantity, 
	...
	return array("plan_order_items" => $query, "total" => $DB->totalRows());
}
