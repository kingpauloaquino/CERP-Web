<?php
   ...
   * Module: WH2 Terminal 
  */
  $capability_key = 'add_product_inventory';
  require('header.php');
	
	if($_POST['action'] == 'add_product_inventory') {
		$id = $DB->Find('warehouse_inventories', array('columns' => 'id', 'conditions'  => 'item_type="PRD" AND item_id='.$_POST['item_id'].' AND 
		...
		if($id==NULL) {
			...
			$invt = array(
	  	  'item_id'		=> $_POST['item_id'],'item_type'	=> $_POST['item_type'], 'invoice_no'=> $_POST['invoice_no'], 'lot_no'		=> $_POST['lot_no'],
	  	  'terminal'	=> 12, 'qty' => 0, 'remarks'		=> $_POST['remarks'] );
			$out_id = $Posts->AddWarehouseInventory($invt);
			
		} else {
			$args = array('type' => $_POST['inventory_type'], 'qty' => $_POST['qty'], 'remarks' => $_POST['remarks'], 'item_type' => 'PRD', 'terminal_id' => $_POST['terminal'],
...
?>
	...
	<div id="page">
        ...
        <h3 class="form-title">Information</h3>
				...
				<input type="hidden" name="item_type" value="PRD"> 
...
<!--           <p class="info"><strong>Notice!</strong> Material codes should be unique.</p> -->
        ...
        <div class="field">
          ...
          <div class="input">
          	<?php
          	$terminals = $DB->Get('terminals', array('columns' => 'id, CONCAT(terminal_code," - ", terminal_name) AS terminal', 'conditions' => 'location_id=3')); // location_id 4 : WIP
...
$(document).ready(function() {
	$('#add-scan-modal').keydown(function(e) { 
		if (e.keyCode == 13) { 
			if($('#barcode').val()!='') {
				get_barcode_data('products', 'products.id, products.product_code AS item_code ', ' ', 'products.bar_code = "' + $('#barcode').val()  + '" ');
			}
