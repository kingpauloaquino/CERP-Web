<?php
  /*
   * Module: Material Inventory History
  */
  $capability_key = 'material_inventory_history';
  require('header.php');
	
	$query_string = extract_querystring($_SERVER["QUERY_STRING"], 'iid,mid');	
	$iid = ($_REQUEST['iid']==NULL)? $_POST['iid'] : $_REQUEST['iid'];
	$mid = ($_REQUEST['mid']==NULL)? $_POST['mid'] : $_REQUEST['mid'];
	
	if(isset($_REQUEST['search'])){
		$cur_search = trim($_REQUEST['search']);
		$cur_filter = $_REQUEST['filter'];
		$query_string = 'iid='.$iid.'&mid='.$mid;
	}	
?>
	<div id="page">
		<div id="page-title">
