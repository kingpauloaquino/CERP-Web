<?php
  ...
  */
  ...
  require('header.php');
	...
	if($_POST['action'] == 'edit_order') {
      ...
      $fields = array('item_id', 'item_type', 'quantity', 'remarks');
		...
		$args = array('variables' => $_POST['order'], 'conditions' => 'id='.$_POST['oid']); 
		$Posts->EditOrder($args);
		
		$ctr=1;
		foreach ($_POST['order_items'] as $arr) {
			if($_POST['remove'][$ctr]=='on') {
				$Posts->RemoveOrderItem(array('conditions' => 'id='.$arr['id']));
			}
			$Posts->EditOrderItem(array('variables' => $arr, 'conditions' => 'id='.$arr['id']));
			$ctr++;
		}
				
		redirect_to($Capabilities->All['show_order']['url'].'?oid='.$_POST['oid']);		
	}
  	...
  	$orders = $DB->Find('orders', array(
		...
		$order_items = $DB->Get('order_items', array(
  			'columns' => 'products.id AS item_id, products.product_code AS item_code, products.description, item_costs.cost, lookups.code AS unit, order_items.item_id, 
  										order_items.item_type, order_items.quantity, (item_costs.cost * order_items.quantity) AS amount, order_items.remarks, order_items.id AS order_item_id', 
  ...
  $roles = $DB->Get('roles', array('columns' => 'id, name', 'conditions' => 'NOT id = 1'));
	$payment_terms = $DB->Get('lookups', array('columns' => 'id, description', 'conditions' => 'parent = "'.get_lookup_code('term_of_payment').'"', 'sort_column' => 'description')); 
?>
	<div id="page">
		<div id="page-title">
    	<h2>
        ...
        <?php
				  ...
				  echo '<a href="'.$Capabilities->All['show_order']['url'].'?oid='.$_REQUEST['oid'].'" class="nav">'.$Capabilities->All['show_order']['name'].'</a>'; 
				?>
				<div class="clear"></div>
      </h2>
		</div>
		...
		<div id="content">			
			<form class="form-container" method="POST">
				...
				<input type="hidden" id="order[client_id]" name="order[client_id]" value="<?php echo $orders['client_id'] ?>"/>
		...
		<!--           <p class="info"><strong>Notice</strong> Message</p> -->
		    ...
		    <div class="field">
		      ...
		      <div class="input">
		        <input type="text" value="<?php echo $orders['client'] ?>" class="magenta" readonly="readonly" />
		      </div>
		      <div class="clear"></div>
		    </div>
		    ...
		    <div class="field">
		      ...
		      <div class="input">
		        <input type="text" id="order[po_number]" name="order[po_number]" value="<?php echo $orders['po_number'] ?>" class="magenta" readonly="readonly" />
		      </div>
		      <div class="clear"></div>
		    </div>
		    ...
		    <div class="field">
		      ...
		      <div class="input">
		        <input type="text" id="order[po_date]" name="order[po_date]" value="<?php echo date("F d, Y", strtotime($orders['po_date'])) ?>" class="datepick" autocomplete="off" />
		      </div>
		      <div class="clear"></div>
		    </div>
		    ...
		    <div class="field">
		      ...
		      <div class="input">
		        <input type="text" id="order[delivery_date]" name="order[delivery_date]" value="<?php echo date("F d, Y", strtotime($orders['delivery_date'])) ?>" class="datepick" autocomplete="off" />
		      </div>
		      <div class="clear"></div>
		    </div>
		    ...
		    <div class="field">
		      ...
		      <div class="input">
		        <input type="text" id="order[terms]" name="order[terms]" value="<?php echo $orders['terms'] ?>" autocomplete="off" />
		      </div>
		      <div class="clear"></div>
		    </div>
		    ...
		    <div class="field">
		      ...
		      <div class="input">
		        <?php select_query_tag($payment_terms, 'id', 'description', $orders['payment_terms'], 'order[payment_terms]', 'order[payment_terms]', '', 'required'); ?>
		      </div>
		      <div class="clear"></div>
		    </div>
          ...
          <div class="input">
            <textarea id="order[description]" name="order[description]"><?php echo $orders['description'] ?></textarea>
          </div>
          <div class="clear"></div>
        </div>        
          ...
          <div class="input">
            <textarea id="order[remarks]" name="order[remarks]"><?php echo $orders['remarks'] ?></textarea>
          </div>
          <div class="clear"></div>
        </div>
				...
				<div class="grid jq-grid">
			      ...
			      <thead>
		            ...
		            <td class="border-right"><a>Remarks</a></td>
								<td width="5%" class="border-right text-center"><a>[ x ]</a></td>
			         </tr>
			      </thead>
			      <tbody>
			      	<?php
			      		...
			      		foreach ($order_items as $item) {
									echo '<tr>';
									echo '<td class="border-right text-right">'.$ctr.'<input type="hidden" id="order_items['.$ctr.'][id]" name="order_items['.$ctr.'][id]" value="'.$item['order_item_id'].'" /></td>';
									...
									echo '<td class="border-right text-right"><input type="text" id="order_items['.$ctr.'][quantity]" name="order_items['.$ctr.'][quantity]" value="'.$item['quantity'].'" onkeyup="add_values()" class="auto_width_right" autocomplete="off"/></td>';
									
									echo '<td class="border-right text-right"><label>'.number_format($item['amount'], 4, '.', '').'</label></td>';
									echo '<td class="border-right"><input type="text" id="order_items['.$ctr.'][remarks]" name="order_items['.$ctr.'][remarks]" value="'.$item['remarks'].'" class="auto_width" autocomplete="off"/></td>';
									
									echo '<td class="border-right text-center"><input type="checkbox" id="remove['.$ctr.']" name="remove['.$ctr.']" class="chk_remove auto_width" /></td>';													
									echo '</tr>';
									$ctr+=1;
								}
			      	 ?>
			...
			<div id="add-order-item-modal" class="modal">
			   ...
			   <div class="content">
			      <form id="add-order-item-form">
			         ...
			         <div class="t-row">
			            <label>Code No:</label>
			            <input type="text" id="item_code" name="item_code" value="" class="searchbox text w180 required"/>
			      			<div id="live_search_display_modal"></div>
			         </div>
...
<script>
	...
	$(document).ready(function(){
		$(".searchbox").keydown(function(e) {
			if (e.keyCode == 9) {
				$('#live_search_display_modal').hide();
			}
		});
		$(".searchbox").keyup(function() {
			...
			if(searchbox=='') {				
				...
				function ajax(ptype, purl, pout){
					$.ajax({
						...
						data: { searchword: searchbox, 
										...
										resultcount: 5,
										table: 			'products',
										columns: 		'products.id, products.product_code AS item_code, product_classification AS class, '+
																'lookups1.description AS unit, lookups2.code AS currency, item_costs.cost, item_costs.item_type ',
										joins: 			'LEFT OUTER JOIN item_costs ON products.id = item_costs.item_id '+
										...
										conditions: 'item_costs.item_type="PRD" AND products.product_code LIKE "%'+searchbox+'%" '+
																'UNION SELECT '+
																...
																'LEFT OUTER JOIN lookups AS lookups1 ON item_costs.unit = lookups1.id '+
																'LEFT OUTER JOIN lookups AS lookups2 ON item_costs.currency = lookups2.id '+
																'WHERE item_costs.item_type="MAT" AND materials.material_code LIKE "%'+searchbox+'%" '
									},
						cache: false,
				...
				ajax ("POST", "../include/livesearch.php", "#live_search_display_modal");
			}
			return false;    
		});			
	});
  ...
  jQuery.fn.add_order_item = function() {
    this.submit(function(e) {
      ...
      var tr				= $('<tr></tr>');
  	  ...
  	  tr.append('<td class="highlight_red border-right text-right"><input type="text" class="auto_width_right" value="'+item_quantity+'" onkeyup="add_values()"/></td>');
  	  tr.append('<td class="highlight_red border-right text-right"><label>'+item_amount.toFixed(4)+'</label></td>');
  	  tr.append('<td class="highlight_red border-right"><input type="text" value="'+item_remarks+'" class="auto_width" /></td>');
  	  tr.append('<td></td>');
  ...
  function add_values() {
      ...
      var total_qty = 0;
      var total_amnt = 0.0000;
	    rows.each(function(index) {
        var qty = parseInt($("td:nth-child(6) input", this).val()) || 0;
        var amnt = (parseFloat($("td:nth-child(5)", this).text()) * qty) || 0.0000; 
        $("td:nth-child(7)", this).text(amnt.toFixed(4));
        total_qty += qty;
        total_amnt += amnt
        $('#total_qty').val(total_qty);
        $('[name*="order[total_quantity]"]').val(total_qty);
        $('#total_amnt').val(total_amnt.toFixed(2));
        $('[name*="order[total_amount]"]').val(total_amnt.toFixed(4));
	    });
  }
  $(".chk_remove").click( function() {
