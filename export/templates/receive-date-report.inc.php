...
/** Error reporting */
  ...
  //ini_set('display_startup_errors', TRUE);
	...
	$objPHPExcel	= $objReader->load("excel_templates/template_received_by_date.xls");
	$objDirectory	= "../export_files";
  $received_items	= $Query->received_by_date_by_month_year($mydate);
...
// 					
  ...
  foreach ($received_items as $item) {
    ...
    $sheet->setCellValue('A'.$row, $r)
          ...
          ->mergeCells('C'.$row.':E'.$row)->setCellValue('C'.$row, $item['supplier_name'])
					->setCellValue('F'.$row, $item['invoice'])
					->setCellValue('G'.$row, $item['receipt']);
