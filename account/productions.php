    ...
    <div id="content">
      ...
      <form id="form-request" action="<?php host($Capabilities->GetUrl()) ?>" method="POST" >
      	...
      	<div id="model-materials-request" style="display: none;">
             ...
             <tr>
                <td>Set total by days:</td><td><input id="prod-days" type="text" class="text-field-medium text-right numeric" value="1" /></td>
                <td>Batch Number:</td><td><input id="batch-no" type="text" class="text-field" placeholder="AUTOGENERATED" readonly/></td>
                <td></td>
             </tr>
             <tr>
	...
	<script>
	  ...
	  $.fn.show_material_requests = function() {
	    ...
	    this.live('click', function(event) {
	    	...
	    	var prod_cp = parseFloat($('.prd-prod-cp').val().replace(/,/g, ''), 10);
	    	//prod_qty = (prod_qty * qty_buffer) + prod_qty;
	    	prod_qty = (prod_qty > prod_cp) ? ((prod_cp * qty_buffer) + prod_cp) : ((prod_qty * qty_buffer) + prod_qty);
