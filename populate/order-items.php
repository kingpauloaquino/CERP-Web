...
function populate_records($keyword='', $page, $limit, $order, $sort) {
  ...
  $startpoint = $limit * ($page - 1);
	...
	$query = $DB->Fetch('order_items', array(
               ...
               'conditions' => 'item_costs.item_type="PRD" AND order_items.item_type="PRD" AND order_items.order_id='. $_GET['order_id'] .'
               							UNION SELECT
			  	  								...
			  	  								INNER JOIN materials ON materials.id=order_items.item_id
														...
														INNER JOIN lookups ON lookups.id = item_costs.unit
														WHERE item_costs.item_type="MAT" AND order_items.item_type="MAT" AND order_items.order_id='. $_GET['order_id']
             ));
