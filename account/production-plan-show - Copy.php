...
?>
	...
	<div id="page">
        ...
        <div class="field">
          ...
          <div class="input">
            <input type="text" name="name" value="<?php echo $prod_detail['po_no'] ?>" readonly="readonly"/>
          </div>
          <div class="clear"></div>
        </div>
        ...
        <div class="field">
          ...
          <div class="input">
            <input type="text" name="name" value="<?php echo date("F d, Y", strtotime($prod_detail['po_date'])) ?>" readonly="readonly"/>
          </div>
          <div class="clear"></div>
        </div>
                
        <div class="field">
          <label class="label">P/O Delivery Date:</label>
          <div class="input">
            <input type="text" name="name" value="<?php echo date("F d, Y", strtotime($prod_detail['delivery_date'])) ?>" readonly="readonly"/>
          </div>
          <div class="clear"></div>
        </div>
        
        <div class="field">
          <label class="label">Production Target Date:</label>
          <div class="input">
            <input type="text" name="name" value="<?php echo date("F d, Y", strtotime($prod_detail['target_date'])) ?>" readonly="readonly"/>
          </div>
          <div class="clear"></div>
        </div>
        ...
        <div class="field">
          ...
          <div class="input">
            <input type="text" name="name" value="<?php echo $prod_detail['status'] ?>" readonly="readonly"/>
          </div>
          <div class="clear"></div>
        </div>
		        	...
		        	<?php
				        	$po_products = $DB->Get('production_purchase_order_products', array(
										  			'columns' 		=> 'production_purchase_order_products.id AS pop_id, production_purchase_order_products.product_id AS pid, production_purchase_order_products.request_id, 
										  												...
										  												products.color AS color, production_purchase_order_products.order_qty AS order_qty, lookups2.description AS type,
									...
									foreach ($po_products as $prod) {
										...
										echo $link = ($prod['lot_no']!=NULL) 
													? '<td class="border-right'.$addtl.'"><a href="production-plan-parts-show.php?'.
															http_build_query(array('ppoid' => $_REQUEST['ppoid'], 'oid' => $_REQUEST['oid'], 'popid' => $prod['pop_id'], 
																'prod_lot_no' => $prod['lot_no'], 'prod' => $prod['product'])).'">'.$prod['product'].'</a></td>'
										...
										echo '<td class="border-right text-right'.$addtl.'">'.$prod['order_qty'].'</td>';
										echo '<td class="border-right text-right'.$addtl.'">'.$prod['prod_qty'].'</td>';
										...
										echo $link = ($prod['lot_no']!=NULL) 
													? '<td class="border-right text-center'.$addtl.'"><a href="production-plan-terminals-show.php?'.
															http_build_query(array('popid' => $prod['pop_id'], 'po' => $prod_detail['po_no'],
																'lot' => $prod['lot_no'], 'prod' => $prod['product'])).'">view</a></td>'
